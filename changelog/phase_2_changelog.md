...
- Constructed the 7-agent sequential handoff graph: `START -> atlas -> scout -> forge -> mosaic -> prism -> compass -> loom -> END`.
- Implemented dynamic router functions (`router_after_atlas`, `router_after_scout`, etc.) enabling agents to be bypassed dynamically based on `agent_plan`.
...
### 2. Atlas Agent (Intent Extractor & Planner)
- **Files**: `backend/prompts/atlas_prompt.py`, `backend/agents/atlas.py`
...
### 3. Scout Agent (Data Scout) & Column Resolver
- **Files**: `backend/tools/column_resolver.py`, `backend/agents/scout.py`
...
### 4. Forge Agent (Feature Engineer) & Composite Feature Registry
- **Files**: `backend/tools/feature_engineering.py`, `backend/agents/forge.py`
...
### 5. Mosaic Agent (Segmentation Agent) & Clustering Models
- **Files**: `backend/models/clustering.py`, `backend/tools/segmentation.py`, `backend/agents/mosaic.py`
...
### 6. Prism Agent (Explainability Agent) & SHAP Engine
- **Files**: `backend/tools/explainability.py`, `backend/agents/prism.py`
...
### 7. Compass Agent (Recommendation Agent) & Banking Product Rules
- **Files**: `backend/tools/recommendation.py`, `backend/agents/compass.py`
...
### 8. Loom Agent (Response Synthesizer & Persona Naming)
- **Files**: `backend/prompts/loom_persona_prompt.py`, `backend/prompts/loom_response_prompt.py`, `backend/agents/loom.py`
...
- Implemented SSE streaming handler (`stream_loom`) rendering chain-of-thought tokens, structured outputs, text chunks, and follow-up suggestion chips.
